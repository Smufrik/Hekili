...
local GetPowerRegenForPowerType = GetPowerRegenForPowerType or function()
	return 0, 0
end
local UnitPartialPower = UnitPartialPower or function()
	return 0
end
local IsSpellKnownOrOverridesKnown = IsSpellKnownOrOverridesKnown or IsSpellKnown
local GetSpellLossOfControlCooldown = function(...)
	return 0, 0
end

-- MoP: Aura functions compatibility
local GetBuffDataByIndex, GetDebuffDataByIndex = function()
	return nil
end, function()
	return nil
end
...
local insert, remove, sort, tcopy, unpack, wipe =
		table.insert, table.remove, table.sort, ns.tableCopy, unpack, table.wipe
...
state.health = {
	...
	timeTo = function(amount)
		if state.health.current >= amount then
			return 0
		end
		return 3600
	end,
...
setmetatable(state.movement, {
	__index = function(t, k)
		if k == "distance" then
			if state.buff.movement.up then
				return state.target.maxR
			end
			return 0
		end
...
local mt_trinket_any_stacking_stat = {
	__index = function(t, k)
		if state.trinket.t1.has_stacking_stat[k] then
			return state.trinket.t1
		elseif state.trinket.t2.has_stacking_stat[k] then
			return state.trinket.t2
		end
		return no_trinket
	end,
}
...
local mt_trinket_any_stat = {
	__index = function(t, k)
		...
		if state.trinket.t1.has_stat[k] then
			return state.trinket.t1
		elseif state.trinket.t2.has_stat[k] then
			return state.trinket.t2
		end
		return no_trinket
	end,
...
local mt_trinket = {
	__index = function(t, k)
		local isEnabled = (not rawget(t, "__usable"))
				or (rawget(t, "__ability") and not state:IsDisabled(t.__ability) or false)
...
local mt_trinket_is = {
	__index = function(t, k)
		...
		if item.usable and item.ability == k then
			return true
		end
		if item.__id and class.gear[k] and class.gear[k][1] == item.__id then
			return true
		end
...
local mt_trinket_has_stacking_stat = {
	__index = function(t, k)
		...
		if not buff or buff.max_stack == 1 then
			return false
		end
...
-- MoP: Loss of Control API not available
state.GetActiveLossOfControlData = function()
	return {}
end
state.GetActiveLossOfControlDataCount = function()
	return 0
end
state.GetNumGroupMembers = GetNumGroupMembers
-- state.GetItemCooldown = GetItemCooldown
-- MoP: Use legacy GetItemCount
state.GetItemCount = (type(_G) == "table" and _G.GetItemCount) or function()
	return 0
end
state.GetItemGem = (type(_G) == "table" and _G.GetItemGem) or function()
	return nil
end
...
state.GetSpellCount = function()
	return 0
end
state.GetSpellInfo = ns.GetUnpackedSpellInfo
-- Guarded WoW API accessors for analyzer environments
state.GetSpellLink = (type(_G) == "table" and _G.GetSpellLink) or function()
	return nil
end
-- MoP: Use legacy GetSpellTexture
state.GetSpellTexture = (type(_G) == "table" and _G.GetSpellTexture) or function()
	return nil
end
state.GetStablePetInfo = (type(_G) == "table" and _G.GetStablePetInfo) or function()
	return nil
end
...
state.IsUsableSpell = (type(_G) == "table" and _G.IsUsableSpell) or function()
	return true
end
state.UnitAura = (type(_G) == "table" and _G.UnitAura) or ns.UnitAura
-- MoP: Aura slots not available
state.UnitAuraSlots = function()
	return {}
end
...
-- MoP: Use fallback for UnitGetTotalAbsorbs
state.UnitGetTotalAbsorbs = UnitGetTotalAbsorbs or function()
	return 0
end
...
-- MoP: Glyph system support
state.glyph = setmetatable({}, {
	__index = function(t, k)
		...
		local glyphsTable = (spec and type(spec.glyphs) == "table") and spec.glyphs
				or (
					Hekili.Class
					and Hekili.Class.specs
					and Hekili.Class.specs[spec.id]
					and Hekili.Class.specs[spec.id].glyphs
				)
		...
		if glyphsTable then
			...
			if glyphID and type(glyphID) == "number" then
				...
				local GetGlyphSocketInfo = (type(_G) == "table" and _G.GetGlyphSocketInfo)
						or function(...)
							return nil
						end
				local GetNumGlyphSockets = (type(_G) == "table" and _G.GetNumGlyphSockets)
						or function()
							return 6
						end
...
-- MoP: Use fallback for UnitPartialPower
...
local function spendCharges(action, charges)
	...
	if not state.cooldown[action] then
...
function state.gainChargeTime(action, time, debug)
	...
	if cooldown.expires > 0 then
		cooldown.expires = max(0, cooldown.expires - time)
	end
end
...
do
	...
	function state.SetupCycle(ability, quiet)
		...
		if not aura and not quiet then
			debug(
				" - no aura identified for target-cycling and no aura matching "
				.. ability.key
				.. " found in ability / spec module; target cycling disabled."
			)
			return
		end
		...
		if cDebuff.up and not ability.cycle_to then
			...
			if state.active_dot[aura] >= state.cycle_enemies then
				if not quiet then
					debug(
						" - we do not have another valid target for "
						.. aura
						.. ": "
						.. state.active_dot[aura]
						.. " vs "
						.. state.cycle_enemies
						.. "."
					)
			...
			if not quiet then
				debug(
					" - we will use the ability on a different target, if available, until %s expires at %.2f [+%.2f].",
					cycle.aura,
					cycle.expires,
					cycle.expires - state.query_time
				)
			end
		elseif
				cDebuff.down
				and ability.cycle_to
				and state.active_dot[aura] > 0
				and state.query_time < state.now + (3 * state.gcd.max)
		then
			cycle.expires = state.query_time +
			(2 * state.gcd.max)                                 -- Assume the aura is available for 3 GCDs (don't forecast a slow target swap).
			cycle.minTTD = max(state.settings.cycle_min, ability.min_ttd or 0, cDebuff.duration / 2)
			cycle.maxTTD = ability.max_ttd
	...
	function state.HasCyclingDebuff(aura)
		...
		if aura and aura ~= cycle.aura then
			return false
		end
		return true
	end
	...
	function state.IsCycling(aura, quiet)
		...
		if state.active_dot[cycle.aura] >= state.cycle_enemies then
			return false,
					format("active_dot[%d] >= cycle_enemies[%d]", state.active_dot[cycle.aura], state.cycle_enemies)
		end
...
local function applyBuff(aura, duration, stacks, value, v2, v3, applied)
	...
	if state.cycle then
		if duration == 0 then
			...
			state.active_dot[aura] = max(state.active_enemies, state.active_dot[aura] + 1)
		end
		return
	end
	...
	if duration == 0 then
		...
		if model.funcs.onRemove then
			model.funcs.onRemove()
		end
	else
		if not b.up or model.friendly then
			state.active_dot[aura] = min(state.group_members, state.active_dot[aura] + 1)
		end
...
local function applyDebuff(unit, aura, duration, stacks, value, noPandemic)
	...
	if state.cycle then
		if duration == 0 then
			if Hekili.ActiveDebug then
				Hekili:Debug("Removed an application of '%s' while target-cycling.", aura)
			end
			state.active_dot[aura] = state.active_dot[aura] - 1
			if model.key ~= aura then
				state.active_dot[model.key] = state.active_dot[aura]
			end
		else
			if Hekili.ActiveDebug then
				Hekili:Debug("Added an application of '%s' while target-cycling.", aura)
			end
			state.active_dot[aura] = state.active_dot[aura] + 1
			if model.key ~= aura then
				state.active_dot[model.key] = state.active_dot[aura]
	...
	if duration == 0 then
		...
		if not noPandemic then
			duration = min(1.3 * duration, d.remains + duration)
		end
...
local function timeToInterrupt()
	...
	if casting.down or casting.v2 == 1 then
		return 3600
	end
	if casting.v3 == 1 then
		return 0
	end
...
function state.stopChanneling(reset, action)
	if not reset then
		...
		if spell then
			if Hekili.ActiveDebug then
				Hekili:Debug("Breaking channel of %s.", spell)
			end
			if ability and ability.breakchannel then
				ability.breakchannel()
			end
	...
	for resource, auras in pairs(class.resourceAuras) do
		if auras.casting then
			state[resource].actual = state[resource].current
		end
	end
...
state.spell_targets = setmetatable({}, {
	__index = function(t, k)
		if state.active_enemies == 1 then
			return 1
		end
		if k == "any" then
		...
		if ability.max_ttd then
			n = min(n, Hekili:GetNumTTDsBefore(ability.max_ttd + state.offset + state.delay))
		end
		if ability.min_ttd then
			n = min(n, Hekili:GetNumTTDsAfter(ability.min_ttd + state.offset + state.delay))
		end
		if ability.max_targets then
			n = min(n, ability.max_targets)
		end
...
state.raid_event.adds = setmetatable({
	...
	__index = function(t, k)
		...
		elseif raid_event_filter[k] ~= nil then
			return raid_event_filter[k]
		end
		...
		if models then
			for k, v in pairs(models) do
				...
				then
					...
					local i = type(v.interval) == "number" and v.interval
							or (
								type(v.interval) == "function" and v.interval(now, r.actual)
								or (type(v.interval) == "string" and state[v.interval] or 0)
							)
			...
			if e.next > finish or not r or not r.actual then
				...
				if stop or aura or channel then
					...
					r.forecast[idx].e = (e.name or "none")
							.. (stop and "-stop" or aura and "-aura" or channel and "-channel" or "-unknown")
					r.fcount = idx
				else
					prev = now
					...
					local step = roundDown(
						type(e.interval) == "number" and e.interval
						or (
							type(e.interval) == "function" and e.interval(now, v)
							or (type(e.interval) == "string" and state[e.interval] or 0)
						),
						3
					)
					...
					if
							e.next > finish
							or step < 0
							or (e.aura and state[e.debuff and "debuff" or "buff"][e.aura].expires < e.next)
							or (e.channel and state.buff.casting.expires < e.next)
					then
						table.remove(events, 1)
					end
				end
...
local resourceChange = function(amount, resource, overcap)
	...
	if amount < 0 and r.spend then
		r.spend(-amount, resource, overcap)
	elseif amount > 0 and r.gain then
		r.gain(amount, resource, overcap)
	else
		r.current = max(0, r.current + amount)
		if not overcap then
			r.current = min(r.max or 100, r.current)
		end
		r.actual = r.current
	end
...
local gain = function(amount, resource, overcap, noforecast)
	...
	local ok = resourceChange(amount, resource, overcap)
	if ok and not noforecast and resource ~= "health" then
...
local spend = function(amount, resource, noforecast)
	...
	local ok = resourceChange(-amount, resource, false)
	if ok and not noforecast and resource ~= "health" then
		forecastResources(resource)
	end
...
do
	...
	recurseRechecks = function(script, seen)
		if not seen then
			...
			seen[script.ID] = script.Recheck or true
		end
				...
				if varIDs then
					for _, entry in ipairs(varIDs) do
						if not seen[entry.id] then
							...
							if subscript then
								recurseRechecks(subscript, seen)
							end
						end
					end
				end
...
do
	...
	local autoReset = setmetatable({
		...
		__index = function(t, k)
			...
			if state.class.stateExprs[k] then
				return 1
			end
			if class.knownAuraAttributes[k] then
				return 1
			end
			if k:match("^time_to_pct") then
				return 1
			end
			if k:match("^incoming_damage") then
				return 1
			end
			if k:match("^incoming_physical") then
				return 1
			end
			if k:match("^incoming_magic") then
				return 1
			end
			if k:match("^incoming_heal") then
				return 1
			end

			-- I should reorder this file to avoid this.
			local x = rawget(t, "GetVariableIDs")
			if x and x(t, k) ~= nil then
				return 1
			end

			x = rawget(t, "settings")
			if x and x[k] ~= nil then
				return 1
			end
	...
	mt_state = {
		__index = function(t, k)
			...
			if state.class.stateExprs[k] then
				return state.class.stateExprs[k]()
			end
			...
			elseif k == "this_action" then
				t[k] = "wait"
			elseif k == "this_list" then
				t[k] = "default"

				-- Calculated from real event data.
			elseif k == "aggro" then
				t[k] = (UnitThreatSituation("player") or 0) > 1
			elseif k == "threat_situation" then
				t[k] = UnitThreatSituation("player") or 0
			elseif k == "threat_percent" then
				local status, percent = UnitThreatSituation("player")
				t[k] = percent or 0
			elseif k == "has_aggro" then
				t[k] = (UnitThreatSituation("player") or 0) >= 3
			elseif k == "losing_aggro" then
				t[k] = (UnitThreatSituation("player") or 0) == 2
			elseif k == "gaining_aggro" then
				t[k] = (UnitThreatSituation("player") or 0) == 1
			elseif k == "no_threat" then
				t[k] = (UnitThreatSituation("player") or 0) == 0
			elseif k == "threat_above_tank" then
				...
				t[k] = playerThreat > 1
			elseif k == "threat_safe" then
				t[k] = (UnitThreatSituation("player") or 0) <= 1
			elseif k == "boss" then
				t[k] = t.encounterID > 0
						or UnitCanAttack("player", "target")
						and (UnitClassification("target") == "worldboss" or UnitLevel("target") == -1)
			elseif k == "encounter" then
				t[k] = t.encounterID > 0
			elseif k == "group" then
				t[k] = t.group_members > 1
			elseif k == "group_members" then
				t[k] = max(1, GetNumGroupMembers())
			elseif k == "active_allies" then
				...
				t[k] = max(1, count)
			elseif k == "level" then
				t[k] = UnitEffectiveLevel("player") or MAX_PLAYER_LEVEL
			elseif k == "mounted" or k == "is_mounted" then
				t[k] = IsMounted()
			elseif k == "moving" then
				t[k] = (GetUnitSpeed("player") > 0)
			elseif k == "raid" then
				t[k] = IsInRaid() and t.group_members > 5
			elseif k == "solo" then
				t[k] = t.group_members == 1
			elseif k == "tanking" then
				t[k] = t.role.tank and t.aggro
			...
			elseif k == "group_health" then
				local UnitGetTotalAbsorbs = UnitGetTotalAbsorbs or function()
					return 0
				end
				local health, maxHealth, absorbs =
						UnitHealth("player"), UnitHealthMax("player"), UnitGetTotalAbsorbs("player")
				local count = 1
				for i = 2, 5 do
					local token = "party" .. i
			...
			elseif k == "at_risk" then
				...
				for i = 2, 5 do
					...
					if UnitExists(token) and not UnitIsDeadOrGhost(token) then
						...
						if (100 * (health + absorbs) / maxHealth) < 70 and 1 or 0 then
							count = count + 1
						end
					end
				end
			...
			elseif k == "active_enemies" then
				...
				if t.min_targets > 0 then
					n = max(t.min_targets, n)
				end
				if t.max_targets > 0 then
					n = min(t.max_targets, n)
				end
				t[k] = max(1, n or 1)
			elseif k == "cycle_enemies" then
				if not t.settings.cycle or t.active_enemies == 1 then
					return 1
				end
				...
				if t.max_targets > 0 then
					targets = min(t.max_targets, targets)
				end
			...
			elseif k == "desired_targets" then
				t[k] = 1
			elseif k == "max_targets" then
				t[k] = 0
			elseif k == "min_targets" then
				t[k] = 0
			...
			elseif k == "channel" then
				if t.buff.casting.down or t.buff.casting.v3 ~= 1 then
					return nil
				end
			...
			elseif k == "channel_breakable" then
				t[k] = false
			...
			elseif k == "cost" then
				...
				if c and c > 0 and c < 1 then
					local mm = t[ability.spendType or class.primaryResource]
							and t[ability.spendType or class.primaryResource].modmax
							or 1
					c = c * mm
				end
				return c or 0
			...
			elseif k == "execute_remains" then
				...
				return (t:IsCasting(action) and max(t:QueuedCastRemains(action), t.gcd.remains))
						or (t.prev[1][action] and t.gcd.remains)
						or 0
			elseif k == "execute_time" then
				return max(t.gcd.execute, ability and ability.cast or 0)
			elseif k == "executing" then
				return t:IsCasting(action) or (t.prev[1][action] and t.gcd.remains > 0)
			elseif k == "full_recharge_time" or k == "time_to_max_charges" then
			...
			elseif k == "in_flight" or k == "in_flight_to_target" then
				return model and model.in_flight or false
			elseif k == "in_flight_remains" then
				return model and model.in_flight_remains or 0
			...
			local app = aura
					and (
						(t.buff[aura_name].up and t.buff[aura_name])
						or (t.debuff[aura_name].up and t.debuff[aura_name])
						or t.buff[aura_name]
					)
			...
			if aura and class.knownAuraAttributes[k] then
				...
				elseif k == "tick_time_remains" then
					return 0
				elseif k == "ticks" then
					return (aura.duration or 15) / (aura.tick_time or (3 * haste))
			...
			if t:GetVariableIDs(k) then
				return t.variable[k]
			end
			if t.settings[k] ~= nil then
				return t.settings[k]
			end
			if t.toggle[k] ~= nil then
				return t.toggle[k]
			end
	...
	mt_pets = {
		__index = function(t, k)
			...
			if not rawget(t, "real_pet") then
				...
				if petID then
					...
					if model then
						...
						local duration = model.duration
								and (type(model.duration) == "function" and model.duration() or model.duration)
								or 3600
			...
			if k == "up" or k == "exists" or k == "active" then
				for k, v in pairs(t) do
					if type(v) == "table" then
						if v.expires > state.query_time then
							return true
						end
					end
				end
			...
			elseif k == "health_pct" or k == "health_percent" then
				if t.alive then
					return 100 * UnitHealth("pet") / UnitHealthMax("pet")
				end
				return 100
			end
...
local mt_stances = {
	__index = function(t, k)
		if not class.stances[k] or not GetShapeshiftForm() then
			return false
		elseif GetShapeshiftForm() < 1 then
			return false
		elseif not select(5, GetShapeshiftFormInfo(GetShapeshiftForm())) == class.stances[k] then
			return false
		end
...
local mt_toggle = {
	__index = function(t, k)
		...
		if
				k == "cooldowns"
				and (toggle.override and state.buff.bloodlust.up or toggle.infusion and state.buff.power_infusion.up)
		then
			return true
		end
...
do
	...
	local PvpDummies = ns.PvpDummies or {}
	local TargetDummies = ns.TargetDummies or {}

	mt_target = {
		__index = function(t, k)
			if k == "distance" then
				t[k] = UnitCanAttack("player", "target") and ((t.minR + t.maxR) / 2) or 7.5
			elseif k == "in_range" then
				return t.distance <= 8
			elseif k == "minR" or k == "maxR" then
				local minR, maxR
				if RC and RC.GetRange then
					minR, maxR = RC:GetRange("target")
				end
				t.minR = minR or 5
				t.maxR = maxR or 10
			...
			elseif k == "exists" then
				t[k] = UnitExists("target")
			elseif k == "health_current" then
				return t.health.current
			elseif k == "health_max" then
				return t.health.max
			elseif k == "health_pct" or k == "health_percent" then
				return t.health.percent
			elseif k == "has_vehicle_ui" then
				t[k] = UnitInVehicle("target")
			elseif k == "is_add" then
				t[k] = not t.is_boss
			elseif k == "is_boss" then
				if
						UnitExists("boss1") and UnitIsUnit("target", "boss1")
						or UnitExists("boss2") and UnitIsUnit("target", "boss2")
						or UnitExists("boss3") and UnitIsUnit("target", "boss3")
						or UnitExists("boss4") and UnitIsUnit("target", "boss4")
						or UnitExists("boss5") and UnitIsUnit("target", "boss5")
				then
					t[k] = true
				else
					t[k] = (
			...
			elseif k == "is_dead" then
				t[k] = UnitIsDeadOrGhost("target")
			elseif k == "is_demon" then
				t[k] = UnitCreatureType("target") == PET_TYPE_DEMON
			elseif k == "is_friendly" then
				t[k] = UnitCanAssist("player", "target")
			elseif k == "is_in_party" then
				t[k] = UnitInParty("target")
			elseif k == "is_in_raid" then
				t[k] = UnitInRaid("target") ~= nil
			elseif k == "is_player" then
				local isPlayer = UnitIsPlayer("target")
				if not isPlayer then
					isPlayer = PvpDummies[t.npcid]
				end
			...
			elseif k == "is_undead" then
				t[k] = UnitCreatureType("target") == BATTLE_PET_NAME_4
			elseif k == "level" then
				t[k] = UnitLevel("target") or UnitLevel("player") or MAX_PLAYER_LEVEL
			elseif k == "moving" then
				t[k] = GetUnitSpeed("target") > 0
			elseif k == "real_ttd" or k == "true_ttd" then
				t[k] = Hekili:GetTTD("target")
			elseif k == "time_to_die" then
				if state.IsCycling() then
					return state.raid_event.adds.remains
				end
				...
				if ttd == 3600 then
					...
					return max(1, t.real_ttd - (state.offset + state.delay))
				end
			elseif k:sub(1, 12) == "time_to_pct_" then
				local percent = tonumber(k:sub(13)) or 0
				return Hekili:GetTimeToPct("target", percent) - (state.offset + state.delay)
			elseif k == "unit" then
				if state.args.cycle_target == 1 then
					return tostring(UnitGUID("target") or "") .. "c" or "cycle"
				elseif state.args.target then
					return tostring(UnitGUID("target") or "") .. "+" .. tostring(state.args.target)
				end
			...
			elseif k == "token" then
				for _, token in ipairs({ "focus", "mouseover", "target", "targettarget" }) do
					if UnitExists(token) and not UnitIsDeadOrGhost(token) and UnitCanAttack("player", token) then
						t[k] = token
						break
					end
				end
...
do
	...
	mt_target_health = {
		__index = function(t, k)
			...
			elseif k == "max" then
				return UnitCanAttack("player", "target") and not UnitIsDead("target") and UnitHealthMax("target")
						or 100000
...
do
	...
	local function GetUnpackedSpellCooldown(spellID)
		-- MoP: Use legacy GetSpellCooldown
		local start, duration, enabled = GetSpellCooldown(spellID)
		local spellCooldownInfo = { startTime = start, duration = duration, isEnabled = enabled }
	...
	mt_default_cooldown = {
		__index = function(t, k)
			...
			local raw = (state.display ~= "Primary" and state.display ~= "AOE")
					or (profile.toggles.cooldowns.value and profile.toggles.cooldowns.separate and noFeignCD)
			...
			if k == "duration" or k == "expires" or k == "charge" or k == "recharge_began" then
				...
				if ability.charges and ability.charges > 1 then
					...
					if not duration then
						duration = max(ability.recharge or 0, ability.cooldown or 0)
					end
			...
			elseif k == "charges" then
				if not raw then
					if state:IsDisabled(t.key) or ability.disabled then
						return 0
					end
				...
				if t.charge == (ability.charges or 1) then
					return t.charge
				end
			...
			elseif k == "time_to_max_charges" or k == "full_recharge_time" then
				...
				return ((ability.charges or 1) - t.true_charges_fractional)
						* max(ability.recharge or ability.cooldown, t.true_duration)
			elseif k == "remains" then
				if t.key == "global_cooldown" then
					return max(0, t.expires - state.query_time)
				end
			...
			elseif k == "charges_fractional" then
				...
				if not ability.charges or t.charge == ability.charges then
					return t.charge or 1
				end
				return min(
					ability.charges,
					t.charge + max(0, (state.query_time - t.recharge_began) / (ability.recharge or ability.cooldown))
				)
			elseif k == "recharge_time" then
				if not ability.charges then
			...
			elseif k == "remains_guess" or k == "remains_expected" then
				...
				if remains == 0 or remains == duration then
					return remains
				end
		...
		__index = function(t, k)
			...
			if not entry then
				...
				if shortkey and class.abilities[shortkey] then
					...
					if not rawget(t, "null_cooldown") then
						t.null_cooldown = { key = "null_cooldown", duration = 1 }
					end
...
local one_sec_gcd = {
	-- MoP: Removed DEMONHUNTER (not available in MoP)
	MONK = true,
	ROGUE = true,
}
...
local mt_gcd = {
	__index = function(t, k)
		if k == "execute" then
			...
			if ability and ability.gcdTime then
				return ability.gcdTime
			end
...
local mt_prev_lookup = {
	__index = function(t, k)
		...
		if t.meta == "castsAll" then
			preds, prev = state.predictions, state.prev
		elseif t.meta == "castsOn" then
			preds, prev = state.predictionsOn, state.prev_gcd
		elseif t.meta == "castsOff" then
			preds, prev = state.predictionsOff, state.prev_off_gcd
		end
	...
	mt_prev = {
		__index = function(t, k)
			if type(k) == "number" then
				...
				return prev_lookup
			elseif k == "last" then
				if t.meta == "castAll" then
					t.last = state.player.lastcast
				elseif t.meta == "castsOn" then
					t.last = state.player.lastgcd
				elseif t.meta == "castsOff" then
					t.last = state.player.lastoffgcd
				end
...
function state:TimeToResource(t, amount)
	...
	if not amount or amount > t.max then
		return 3600
	end
	if t.current >= amount then
		return 0
	end
	...
	if t.forecast and t.fcount > 0 then
		if t.times[amount] then
			return t.times[amount] - queryTime
		end
...
local mt_resource = {
	__index = function(t, k)
		...
		elseif k == "time_to_max_combined" then
			if not state.spec.assassination then
				return t.time_to_max
			end
...
do
	local autoReset = setmetatable({
		...
		__index = function(t, k)
			if class.knownAuraAttributes[k] ~= nil then
				return 1
			end
		end,
	})
	...
	mt_alias_buff = {
		__index = function(t, k)
			...
			if aura.meta and aura.meta[k] then
				return aura.meta[k]()
			end

			if k == "max_stack" then
				return aura.max_stack or 1
			elseif k == "count" or k == "stack" or k == "stacks" then
				local n = 0

				if type == "any" then
					for i, child in ipairs(aura.alias) do
						if state.buff[child].up then
							n = n + max(1, state.buff[child].stack)
						end
						if state.debuff[child].up then
							n = n + max(1, state.debuff[child].stack)
						end
					end
				else
					for i, child in ipairs(aura.alias) do
						if state[type][child].up then
							n = n + max(1, state[type][child].stack)
						end
					end
				end
			...
			for i, v in ipairs(aura.alias) do
				...
				if child.up then
					if mode == "shortest" and (not alias or child.remains < alias.remains) then
						alias = child
					elseif mode == "longest" and (not alias or child.remains > alias.remains) then
						alias = child
					end
				end
			end
			...
			if alias then
				...
				return state[type][aura.alias[1]][k]
			end
		end,
		__newindex = function(t, k, v)
			if v == nil then
		...
		__index = function(t, k)
			...
			if aura and aura.hidden then
				...
				local name, _, count, _, duration, expires, caster, _, _, spellID, _, _, _, _, timeMod, v1, v2, v3 =
						state.FindPlayerAuraByID(aura.id)
			...
			elseif k == "last_trigger" then
				if state.combat > 0 then
					return max(0, t.last_application - state.combat)
				end
				return 0
			elseif k == "last_expire" then
				if state.combat > 0 and t.last_expiry < state.query_time then
					return max(0, t.last_expiry - state.combat)
				end
				return 0
			else
...
local mt_default_talent = {
	__index = function(t, k)
		if k == "i_enabled" or k == "rank" then
			return t.enabled and 1 or 0
		end
		return k
	end,
}
...
local mt_default_pvptalent = {
	__index = function(t, k)
		...
		if k == "enabled" then
			return enlisted and rawget(t, "_enabled") or false
		elseif k == "_enabled" then
			return false
		elseif k == "i_enabled" or k == "rank" then
			return enlisted and rawget(t, "_enabled") and 1 or 0
		end
...
do
	...
	state.variable = setmetatable({}, {
		__index = function(t, var)
			...
			if not db[var] then
				if debug then
					Hekili:Debug("No such variable '%s'.", var)
				end
			...
			if not parent or parent == "NilScriptID" then
				return 0
			end
			...
			for i, entry in ipairs(data) do
				...
				if pathState[currPath] then
					...
					if cache[var][pathKey] ~= nil then
						...
						if debug then
							conditions =
									format("%s: %s", passed and "PASS" or "FAIL", scripts:GetConditionsAndValues(scriptID))
							valueString = format(
								"%s: %s",
								value ~= nil and tostring(value) or "nil",
								scripts:GetModifierValues("value", scriptID)
							)
...
do
	...
	mt_default_debuff = {
		...
		__index = function(t, k)
			...
			local remains = meta and meta.remains and meta.remains(t, "debuff")
					or applied > 0 and applied <= moment and expires > moment and expires - moment
					or 0
			...
			if k == "i_up" or k == "rank" then
				return remains > 0 and 1 or 0
			end

			if k == "stack" or k == "stacks" or k == "react" then
				return remains > 0 and t.count or 0
			end

			if k == "max_stack" or k == "max_stacks" then
				return max(t.count, aura.max_stack or 1)
			end

			if k == "at_max_stacks" then
				return remains > 0 and t.count >= (aura.max_stack or 1)
			end
			if k == "stack_pct" then
				return remains > 0 and (100 * t.count / (aura.max_stack or 1)) or 0
			end
			if k == "value" then
				return remains > 0 and t.v1 or 0
			end
			if k == "stack_value" then
			...
			if k == "apply_duration" then
				return apply_duration
			end
			if k == "refreshable" then
				return remains < 0.3 * apply_duration
			end
			if k == "time_to_refresh" then
				return remains > 0 and max(0, 0.01 + remains - (0.3 * apply_duration)) or 0
			end
			...
			if k == "ticks" then
				return remains > 0 and floor((last_tick - applied) / tick_time) or 0
			end

			local next_tick = remains > 0 and max(ns.GetDebuffNextTick(t.key, state.target.unit), last_tick + tick_time)
					or 0
			...
			if k == "tick_time_remains" then
				return max(0, next_tick - moment)
			end
			if k == "ticks_remain" then
				return expires > moment and max(0, 1 + floor((expires - next_tick) / tick_time)) or 0
			end
		...
		__index = function(t, k)
			...
			if aura then
				...
				if Hekili.PLAYER_ENTERING_WORLD and not debuffs_warned[k] then
					local key_str = type(k) == "function" and tostring(k) or k
					Hekili:Error(
						"WARNING: Unknown debuff in ["
						.. (state.scriptID or "unknown")
						.. "]: "
						.. key_str
						.. "\n\n"
						.. debugstack()
...
local mt_default_action = {
	__index = function(t, k)
		...
		elseif k == "ticks" then
			return 1
					+ (state.dot[aura].duration or (30 * state.haste) / class.auras[aura].tick_time or (3 * state.haste))
					- t.ticks_remain
		elseif k == "ticks_remain" then
		...
		elseif k == "cost" then
			if ability then
				...
				if c and c > 0 and c < 1 then
					local mm2 = state[ability.spendType or class.primaryResource]
							and state[ability.spendType or class.primaryResource].modmax
							or 1
					c = c * mm2
				end
		...
		elseif k == "in_flight_remains" then
			...
			return max(
				state:InFlightRemains(t.action),
				ability.isProjectile and ability.lastCast + 0.25 - state.query_time or 0
			)
		elseif k == "channeling" then
		...
		elseif k == "execute_remains" then
			return (state:IsCasting(t.action) and max(state:QueuedCastRemains(t.action), state.gcd.remains))
					or (state.prev[1][t.action] and state.gcd.remains)
					or 0
		...
		elseif k == "in_range" then
			if
					...
					and LSR.IsSpellInRange(ability.rangeSpell or ability.id, "target") == 0
			then
				return false
			end
...
-- Ensure class.specs is initialized
if not class.specs then
	class.specs = {}
end
...
do
	...
	function state.StoreMatchingAuras(unit, auras, filter, ...)
		...
		for i = select("#", ...), 1, -1 do
			local slot = select(i, ...)
			local name, _, count, _, duration, expires, caster, _, _, spellID, _, _, _, _, timeMod, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10 =
			...
			if
					key
					and name
					and (unit == "player" or caster and (UnitIsUnit(caster, "player") or UnitIsUnit(caster, "pet")))
			then
				local a = db[key]
...
function state.putTrinketsOnCD(val)
	...
	for i, item in ipairs(state.items) do
		if not class.abilities[item].no_icd and state.cooldown[item].remains < val then
...
do
	...
	function state:QueueEvent(action, start, time, type, target, real)
		...
		if not time then
			...
			if ability then
				if type == "PROJECTILE_IMPACT" then
					if ability.flightTime then
						time = start + 0.05 + ability.flightTime
					else
						time = start + 0.05 + (state.target.maxR / ability.velocity)
					end
		...
		if action and start and time and type then
			...
			if Hekili.ActiveDebug and not real then
				Hekili:Debug("Queued %s from %.2f to %.2f (%s).", action, start, time, type)
			end
		end
	end
	...
	function state:QueueAuraEvent(action, func, time, eType, data)
		...
		if Hekili.ActiveDebug then
			Hekili:Debug("Queued %s %s at +%.2f.", action, eType, time - state.query_time)
		end
	end
	...
	function state:GetEventInfo(action, start, time, type, target, real)
		...
		if real and (queue[action] or 0) == 0 then
			return
		end
		...
		for i, event in ipairs(queue) do
			if
					(not action or event.action == action)
					and (not start or event.start == start)
					and (not time or event.time == time)
					and (not type or event.type == type)
					and (not target or event.target == target)
			then
	...
	function state:HandleEvent(e)
		...
		if Hekili.ActiveDebug then
			Hekili:Debug("\nHandling %s at %.2f (%s).", action, e.time, e.type)
		end
		...
		if e.type == "CAST_FINISH" then
			...
			if e.target and e.target ~= self.target.unit then
				if Hekili.ActiveDebug then
					Hekili:Debug("Using ability on a different target.")
				end
			...
			if ability.item and not ability.no_icd then
				self.putTrinketsOnCD(cooldown / 6)
			end
		elseif e.type == "CHANNEL_TICK" then
			if ability.tick then
				ability.tick()
			end
		...
		elseif e.type == "PROJECTILE_IMPACT" then
			...
			if ability.impact then
				ability.impact(e.real)
			end
			self:StartCombat()
	...
	function state:IsQueued(action, real)
		if real and (realQueue[action] or 0) == 0 then
			return false
		end
	...
	function state:InFlightRemains(action, real)
		...
		for i, entry in ipairs(queue) do
			if entry.action == action and entry.type == "PROJECTILE_IMPACT" and entry.start <= self.query_time then
				return max(0, entry.time - self.query_time)
			end
		end
	...
	function state:IsCasting(action, real)
		...
		for i, entry in ipairs(queue) do
			if
					entry.type == "CAST_FINISH"
					and (action == nil or entry.action == action)
					and entry.start <= self.query_time
			then
				return true
			end
		end
	...
	function state.reset(dispName, full)
		...
		if display then
			if dispName == "Primary" then
				if mode == "single" or mode == "dual" or mode == "reactive" then
					state.max_targets = 1
			...
			elseif dispName == "AOE" then
				state.min_targets = spec and spec.aoe or 3
				state.filter = "none"
			elseif dispName == "Cooldowns" then
				state.filter = "cooldowns"
			elseif dispName == "Interrupts" then
				state.filter = "interrupts"
			elseif dispName == "Defensives" then
				state.filter = "defensives"
			end
		...
		if not full then
			...
			if displayFrame.resetShortN then
				displayFrame.resetShortMean = (
					displayFrame.resetShortMean * displayFrame.resetShortN + displayFrame.resetShortLast
		...
		if state.time == 0 and InCombatLockdown() then
			...
			if a and a.startsCombat and state.now - a.lastCast < 1 then
				...
				if Hekili.ActiveDebug then
					Hekili:Debug(
						format(
							"Starting combat based on %s cast; time is now: %.2f.",
							state.player.lastcast,
							state.time
						)
					)
				end
		...
		for k, power in pairs(class.resources) do
			...
			if res then
				...
				-- MoP: Use legacy power type constants
				if power.type == 0 then -- MANA = 0 in MoP
		...
		if not foundResource then
			...
			if displayFrame.resetAbortN then
				displayFrame.resetAbortMean = (
					displayFrame.resetAbortMean * displayFrame.resetAbortN + displayFrame.resetAbortLast
		...
		state.nextMH = (
			state.combat > 0
			and state.swings.mh_actual > state.combat
			and state.swings.mh_actual + state.mainhand_speed
		) or 0
		state.nextOH = (
			state.combat > 0
			and state.swings.oh_actual > state.combat
			and state.swings.oh_actual + state.offhand_speed
		) or 0
		...
		if casting and cast_time > 0 then
			...
			if ability then
				...
				destGUID = Hekili:GetMacroCastTarget(ability.key, state.buff.casting.applied, "RESET")
						or state.target.unit
			end

			if not state:IsCasting() and not channeled then
				state:QueueEvent(
					casting,
					state.buff.casting.applied,
					state.buff.casting.expires,
					"CAST_FINISH",
					destGUID
				)
			...
			if dispName == "Primary" or dispName == "AOE" then
				...
				if delay > 0 then
					if Hekili.ActiveDebug then
						Hekili:Debug("Advancing by %.2f per GCD or cast or channel or reset_postcast value.", delay)
					end
					state.advance(delay)
				end
			end
		...
		if displayFrame.resetFullN then
			displayFrame.resetFullMean = (
				displayFrame.resetFullMean * displayFrame.resetFullN + displayFrame.resetFullLast
...
function state:StartCombat()
	if self.time == 0 then
		...
		if Hekili.ActiveDebug then
			Hekili:Debug(format("Starting combat at %.2f -- time is %.2f.", self.false_start, self.time))
		end
	end
	...
	if swing then
		self:ForecastSwingbasedResources()
	end
end
...
function state.advance(time)
	...
	while event do
		if event.time <= state.query_time + time then
			...
			if Hekili.ActiveDebug then
				Hekili:Debug(
					"While advancing by %.2f to %.2f, %s %s occurred at %.2f.",
					time,
					realOffset + time,
					event.action,
					event.type,
...
ns.spendResources = function(ability)
	...
	if action.spend ~= nil then
		...
		if cost ~= 0 and resource and state[resource] then
...
do
	...
	function Hekili:PlaceHold(action, combat, verbose)
		...
		if ability then
			...
			if verbose then
				Hekili:Print(
					class.abilities[ability.key].name
					.. " placed on hold"
					.. (combat and " until end of combat." or ".")
				)
			end
	...
	function Hekili:RemoveHold(action, verbose)
		...
		if ability and state.holds[ability.key] then
			...
			if verbose then
				Hekili:Print(class.abilities[ability.key].name .. " hold removed.")
			end
	...
	function Hekili:ReleaseHolds(combat)
		...
		if holdRemoved then
			Hekili:ForceUpdate("HEKILI_COMBAT_HOLD_REMOVED")
		end
	end
end
...
function state:IsKnown(sID)
	...
	if not ability then
		Error(
			"IsKnown() - " .. tostring(sID) .. " / " .. original .. " not found in abilities table.\n\n" .. debugstack()
		)
	...
	if sID < 0 then
		...
		if ability.item and ability.key ~= "potion" then
			return IsUsableItem(ability.item),
					"IsUsableItem item " .. ability.item .. " and " .. (tostring(ability.known) or "nil")
		end
		return true
	end
...
do
	...
	function state:IsDisabled(spell, strict)
		...
		if not strict then
			...
			if toggle == "potion" and profile.toggles[toggle].separate and not profile.toggles[toggle].value then
				toggle = "cooldowns"
			end
			...
			if
					toggle
					and toggle ~= "none"
					and (not self.toggle[toggle] or (profile.toggles[toggle].separate and state.filter ~= toggle))
			then
				return true, format("toggle %s", toggle)
			end
		...
		if ability.disabled then
			return true, "disabled per ability function"
		end
		if self.holds[spell] then
			return true, "on hold"
		end
		...
		if dotCap and dotCap > 0 and class.auras[spell] then
			...
			if activeDot and activeDot >= dotCap then
				if aura and not aura.refreshable then
					return true,
							"applications[" .. activeDot .. "] meet/exceed the ability's maximum targets [" .. dotCap .. "]"
				end
			end
			...
			if Hekili.ActiveDebug then
				if activeDot < dotCap then
					Hekili:Debug(
						"    %s: current applications fewer than maximum applications [%d/%d].",
						spell,
						activeDot,
						dotCap
					)
				elseif aura and aura.refreshable then
					Hekili:Debug(
						"    %s: target refreshable despite maximum applications [%d/%d]",
						spell,
						activeDot,
						dotCap
					)
	...
	function state:IsFiltered(spell)
		...
		if toggleSpells[spell] and toggle and toggle ~= "none" then
			if
					...
					or (profile.toggles[toggle].separate and state.filter ~= toggle and not spec.noFeignedCooldown)
			then
				return true, format("%s filtered", toggle)
			end
		end
	...
	function state:IsUsable(spell)
		...
		if ability.channeling then
			...
			if
					not c
					or state.buff.casting.remains < 0.1
					or state.buff.casting.v3 ~= 1
					or state.buff.casting.v1 ~= c
			then
				return false,
						"required channel ("
						.. c
						.. " / "
						.. ability.channeling
						.. ") not active or too short [ "
						.. state.buff.casting.remains
						.. " / "
						.. state.buff.casting.applied
						.. " / "
						.. state.buff.casting.expires
						...
						.. state.query_time
						.. " / "
						.. tostring(state.buff.casting.v3)
						.. " / "
						.. state.buff.casting.v1
						.. " ]"
			end
		end
...
function state:TimeToReady(action, pool)
	...
	if z and z > wait then
		if Hekili.ActiveDebug then
			Hekili:Debug(
				"ICD is " .. z .. ", last cast was " .. lastCast .. ", remaining CD: " .. max(0, lastCast + z - now)
			)
		end
		wait = z
	end
...
function state:IsReady(action)
	...
	if ability.spend then
		...
		if resource == "focus" or resource == "energy" or state.script.entry then
			...
			if ttr < self.delayMin or ttr > self.delayMax then
				return false, format("not ready within  time contraint ( %.2f - %.2f )", self.delayMin, self.delayMax)
			elseif ttr >= self.selection_time then
				return false,
						format(
							"not ready [%.2f] before selected action %s [%.2f]",
							ttr,
							self.selected_action or "no_action",
							self.selection_time
						)
			end
...
function state:IsReadyNow(action)
	...
	if self.cooldown[action].remains - clash > 0 then
		return false
	end
	...
	if a.ready and type(a.ready) == "function" and a.ready() > 0 then
		return false
	end
