...
local BOOKTYPE_SPELL = BOOKTYPE_SPELL or "spell"  -- MoP compatibility
-- MoP: Create fallbacks for modern APIs
local GetSpellBookItemName = GetSpellBookItemName or function(index, spellBank)
  return GetSpellName(index, spellBank)
end
-- MoP: C_Item fallback
...
-- MoP: Create spell range check fallback
local IsSpellBookItemInRange = _G.IsSpellInRange or function(index, spellBank, unit)
  -- Simple fallback - assume in range if function doesn't exist
  return 1
end
...
local GetSpellBookItemInfo = _G.GetSpellBookItemInfo or function(index, spellBank)
  ...
  return "SPELL", nil, { spellType = "SPELL", spellID = nil }
end
...
for _, n in ipairs({ "DEATHKNIGHT", "DRUID", "HUNTER", "SHAMAN", "MAGE", "PALADIN", "PRIEST", "WARLOCK", "WARRIOR", "MONK", "ROGUE" }) do
  FriendSpells[n], HarmSpells[n], ResSpells[n], PetSpells[n] = {}, {}, {}, {}
end

-- MoP: Removed Evoker and Demon Hunter spell setups (not available in MoP)
