...
local state = read_file("State.lua")

expect_contains(toc, "## Interface: 50504", "TOC interface must target WoW 5.5.4.")
expect_contains(toc, "## Version: v5.5.4", "TOC version should stay on the client interface version.")
expect_contains(toc, "## Title: Hekili", "TOC title should stay unchanged.")
expect_contains(core, 'Hekili.Version = "v5.5.4-3"', "Addon runtime version must identify the 5.5.4 build.")
...
expect_contains(state, 'ret_tier15 = "tier15"', "Ret T15 set-bonus keys must fall back to the generic tier15 gear count.")
expect_contains(state, 'ret_tier16 = "tier16"', "Ret T16 set-bonus keys must fall back to the generic tier16 gear count.")
expect_contains(state, "local equipped = rawget( t, set ) or 0", "Set-bonus aliases must read the actual tracked gear count.")
