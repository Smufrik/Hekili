...
local GetSpellBookItemInfo = function(index, bookType)
    if C_SpellBook and C_SpellBook.GetSpellBookItemInfo then
        -- MoP: Use legacy constants instead of Enum
        local spellBank = ( bookType == "spell" or bookType == 2 ) and 0 or 1 -- PLAYER = 0, PET = 1
        local info = C_SpellBook.GetSpellBookItemInfo(index, spellBank)
        if info then return info.name, info.iconID, info.spellID end
    else
        -- MoP: Use legacy API
        return _G.GetSpellBookItemInfo(index, bookType)
    end
end
...
-- MoP: C_Spell.GetSpellCooldown not available, use GetSpellCooldown
local GetSpellCooldown = C_Spell and C_Spell.GetSpellCooldown or _G.GetSpellCooldown

-- MoP: C_Spell.GetSpellDescription not available, use GetSpellDescription
local GetSpellDescription = GetSpellDescription or function() return "" end

-- MoP: C_Spell.GetSpellCharges not available, create compatibility function
local GetSpellCharges = function(spellID)
    -- MoP doesn't have spell charges system, return nil for all spells
...
function SkeletonGen:PrepareSpecData()
    ...
    -- MoP: PvP talents don't exist, skip this section
    if C_SpecializationInfo and C_SpecializationInfo.GetPvpTalentSlotInfo then
        ...
        if slotInfo then
            for _, tID in ipairs( slotInfo.availableTalentIDs ) do
                ...
                self.pvptalents[ token ] = {                    talent = tID,
                    id = spellID,
                    tooltip = tooltip,
                }
    ...
    end -- MoP: Close the C_SpecializationInfo check
        ...
        for i = 1, count do
            ...
            if name and spellID and not IsPassiveSpell( spellID ) and IsPlayerSpell( spellID ) then
                ...
                if HasText ( desc ) then
                    ability.tooltip = self:CleanTooltip( desc )                end
                self.idToToken[ spellID ] = token
                self.abilities[ token ] = self:EmbedSpellData( spellID, token, ability )
            end
        end
...
function SkeletonGen:Generate()
    ...
    do
        ...
        self:Append( "" )        self:Append( string.format( "spec:RegisterPack( \"%s\", %d, [[%s]] )", specName, today, export ) )
    end
