...
function Hekili:OnInitialize()
    ...
    local function GetDataText()
        ...
        return format("|c%s%s|r %sCD|r %sInt|r %sDef|r", color,
            m == "single" and "ST" or
            (m == "aoe" and "AOE" or (m == "dual" and "Dual" or (m == "reactive" and "React" or "Auto"))),
    ...
    -- MoP: AddonCompartmentFrame doesn't exist, skip registration
    ...
    if LDB then
        ...
        if LDBIcon then
            LDBIcon:Register("Hekili", ns.UI.Minimap, Hekili.DB.profile.iconStore)
        end
    end
...
function Hekili:PopStack()
    ...
    for i = #Block, 1, -1 do
        if Block[i].parent == x.script then
            if self.ActiveDebug then self:Debug("Removed " ..
                Block[i].list .. " from blocklist as " .. x.list .. " was its parent.") end
        ...
        if scripts:CheckScript(x.script) then
            ...
            if script.Recheck then
                ...
                if Hekili.ActiveDebug then Hekili:Debug(
                    " - blocking script did not immediately block; will attempt to tune it.") end
                ...
                if firstFail and firstFail > 0 then
                    ...
                    for i = 1, 10 do
                        if subFail then
                            subFail = firstFail - (firstFail - subFail) / 2
                        else
                            subFail = firstFail / 2
                        end
                    ...
                    state.delayMin = actualDelay + firstFail
                    if Hekili.ActiveDebug then Hekili:Debug(" - setting delayMin to " ..
                        state.delayMin .. " based on recheck and brute force.") end
                else
                    state.delayMin = x.priorMin
                    -- Leave it alone.
...
function Hekili:CheckStack()
    ...
    for i, b in ipairs(Block) do
        ...
        if self.ActiveDebug then
            ...
            self:Debug("Blocking list ( %s ) called from ( %s ) would %s at %.2f.", b.list, b.script,
                cache[t] and "BLOCK" or "NOT BLOCK", state.delay)
            self:Debug(values[t])
        end
    ...
    for i, s in ipairs(Stack) do
        ...
        if self.ActiveDebug then
            ...
            self:Debug("List ( %s ) called from ( %s ) would %s at %.2f.", s.list, s.script,
                cache[t] and "PASS" or "FAIL", state.delay)
...
function Hekili:CheckChannel(ability, prio)
    ...
    if not a.tick_time and (not aura or not aura.tick_time) then
        if self.ActiveDebug then self:Debug(
            "CC: No aura / no aura.tick_time to forecast channel breaktimes; don't break it.") end
        return false
    end
    ...
    if channel ~= ability then
        ...
        if modifiers.interrupt_if and modifiers.interrupt_if() then
            ...
            if self.ActiveDebug then self:Debug("CC: Interrupt_If - GCD: %s, Immediate: %s, Timing: %s.",
                    tostringall(gcd, imm, timing)) end
...
function Hekili:GetPredictionFromAPL(dispName, packName, listName, slot, action, wait, depth, caller)
    ...
    local pack
    if (packName == "UseItems") then
        pack = class.itemPack
    else
        pack = self.DB.profile.packs[packName]
    end
    ...
    if self:IsListActive(packName, listName) then
        ...
        while actID <= #list do
            ...
            if rWait <= 0.2 then
                if debug then self:Debug("The recommended action (%s) is ready in less than 0.2s; exiting list (%s).",
                        rAction, listName) end
                break
            elseif state.delayMin > state.delayMax then
                if debug then self:Debug(
                    "The current minimum delay (%.2f) is greater than the current maximum delay (%.2f). Exiting list (%s).",
                        state.delayMin, state.delayMax, listName) end
                break
            elseif rAction and not packInfo.hasOffGCD and rWait <= state.cooldown.global_cooldown.remains then -- and state.settings.gcdSync then
                if debug then self:Debug(
                    "The recommended action (%s) is ready within the active GCD; exiting list (%s).", rAction, listName) end
                break
            elseif rAction and state.empowering[rAction] then
                if debug then self:Debug("The recommended action (%s) is currently empowering; exiting list (%s).",
                        rAction, listName) end
                break
            elseif stop then
            ...
            if self:IsActionActive(packName, listName, actID) then
                ...
                elseif state.whitelist and not state.whitelist[action] and (ability.id < -99 or ability.id > 0) then
                    if debug then self:Debug("[---] %s ( %s - %d) not castable while casting a spell; skipping...",
                            action, listName, actID) end
                elseif rWait <= state.cooldown.global_cooldown.remains and not state.spec.can_dual_cast and ability.gcd ~= "off" then
                    if debug then self:Debug(
                        "Only off-GCD abilities would be usable before the currently selected ability; skipping...") end
                else
                    local entryReplaced = false
                    if action == "trinket1" then
                        if state.trinket.t1.usable and state.trinket.t1.ability then -- and not Hekili:IsItemScripted( state.trinket.t1.ability, true ) then
                            ...
                            if debug then
                                self:Debug("\nBypassing 'trinket1' action because %s.",
                                    state.trinket.t1.usable and state.trinket.t1.ability and
                                    (state.trinket.t1.ability .. " is used elsewhere in this priority") or
                                    "the equipped trinket #1 is not usable")
                            end
                            ability = nil
                        end
                    elseif action == "trinket2" then
                        if state.trinket.t2.usable and state.trinket.t2.ability then -- and not Hekili:IsItemScripted( state.trinket.t2.ability, true ) then
                            ...
                            if debug then
                                self:Debug("\nBypassing 'trinket2' action because %s.",
                                    state.trinket.t2.usable and state.trinket.t2.ability and
                                    (state.trinket.t2.ability .. " is used elsewhere in this priority") or
                                    "the equipped trinket #2 is not usable")
                            end
                            ability = nil
                        end
                    ...
                    elseif action == "potion" then
                        ...
                        if not usePotion or not class.abilities[usePotion] then usePotion = class.specs[specID].options
                            .potion end
                    ...
                    if debug then
                        ...
                        if entryReplaced then d = format(
                            "\nSubstituting %s for %s action; it is otherwise not included in the priority.", action,
                                class.abilities[entry.action].name) end

                        if action == "call_action_list" or action == "run_action_list" then
                            d = d ..
                            format("\n%-4s %s ( %s - %d )", rDepth .. ".",
                                (action .. ":" .. (state.args.list_name or "unknown")), listName, actID)
                        elseif action == "cancel_buff" then
                            d = d ..
                            format("\n%-4s %s ( %s - %d )", rDepth .. ".",
                                (action .. ":" .. (state.args.buff_name or "unknown")), listName, actID)
                        elseif action == "cancel_action" then
                            d = d ..
                            format("\n%-4s %s ( %s - %d )", rDepth .. ".",
                                (action .. ":" .. (state.args.action_name or "unknown")), listName, actID)
                        else
                            d = d .. format("\n%-4s %s ( %s - %d )", rDepth .. ".", action, listName, actID)
                        end

                        if not enabled then
                            d = d .. " - ability disabled ( " .. (enReason or "unknown") .. " )"
                        elseif not known then
                            d = d .. " - " .. (reason or "ability unknown")
                        end
                    ...
                    if ability and known and enabled then
                        ...
                        elseif wait_time > state.delayMax then
                            if debug then self:Debug(
                                "The action is not ready ( %.2f ) before our maximum delay window ( %.2f ) for this query.",
                                    wait_time, state.delayMax) end
                        elseif (rWait - state.ClashOffset(rAction)) - (wait_time - clash) <= 0.05 then
                            if debug then self:Debug(
                                "The action is not ready in time ( %.2f vs. %.2f ) [ Clash: %.2f vs. %.2f ] - padded by 0.05s.",
                                    wait_time, rWait, clash, state.ClashOffset(rAction)) end
                        else
                            -- APL checks.
                            if precombatFilter and not ability.essential then
                                if debug then self:Debug(
                                    "We are already in-combat and this pre-combat action is not essential.  Skipping.") end
                            else
                                ...
                                if action == "call_action_list" or action == "run_action_list" or action == "use_items" then
                                    ...
                                    if strict_if then
                                        if debug then self:Debug("Strict Conditions [%s] for %s were not met.",
                                                entry.strict_if,
                                                action == "use_items" and "Use Items" or state.args.list_name or
                                                "this action list") end
                                    elseif not entry.criteria or entry.criteria == "" then
                                        if debug then self:Debug("There is no criteria for %s.",
                                                action == "use_items" and "Use Items" or state.args.list_name or
                                                "this action list") end
                                        ...
                                        if not aScriptPass and ts then
                                            ...
                                            if #state.recheckTimes == 0 then
                                                if debug then self:Debug(
                                                    "Time-sensitive Criteria FAIL at +%.2f with no valid rechecks - %s",
                                                        state.offset, scripts:GetConditionsAndValues(scriptID)) end
                                                ts = false
                                            elseif cap_sec > 0 and state.recheckTimes[1] > cap_sec then
                                                if debug then self:Debug(
                                                    "Time-sensitive Criteria FAIL at +%.2f with first rechecks exceeding preferred Maximum Forecasting Time ( %.2f > %.2f ) - %s",
                                                        state.offset, state.recheckTimes[1], cap_sec,
                                                        scripts:GetConditionsAndValues(scriptID)) end
                                                ts = false
                                            elseif state.recheckTimes[1] > rWait then
                                                if debug then self:Debug(
                                                    "Time-sensitive Criteria FAIL at +%.2f with rechecks greater than wait time ( %.2f > %.2f ) - %s",
                                                        state.offset, state.recheckTimes[1], rWait,
                                                        scripts:GetConditionsAndValues(scriptID)) end
                                                ts = false
                                            elseif state.delayMax and state.recheckTimes[1] > state.delayMax then
                                                if debug then self:Debug(
                                                    "Time-sensitive Criteria FAIL at +%.2f with rechecks outside of max delay ( %.2f > %.2f ) - %s",
                                                        state.offset, state.recheckTimes[1], state.delayMax,
                                                        scripts:GetConditionsAndValues(scriptID)) end
                                                ts = false
                                            end
                                        else
                                            if debug then
                                                self:Debug("%sCriteria for %s %s at +%.2f - %s",
                                                    ts and "Time-sensitive " or "", state.args.list_name or "???",
                                                    ts and "deferred" or (aScriptPass and "PASS" or "FAIL"), state
                                                .offset, scripts:GetConditionsAndValues(scriptID))
                                            end
                                        end
                                    ...
                                    if aScriptPass then
                                        if action == "use_items" then
                                            ...
                                            rAction, rWait, rDepth = self:GetPredictionFromAPL(dispName, "UseItems",
                                                "items", slot, rAction, rWait, rDepth, scriptID)
                                            if debug then self:Debug(
                                                "Returned from Use Items; current recommendation is %s (+%.2f).",
                                                    rAction or "NO ACTION", rWait) end
                                            self:PopStack()
                                        else
                                            ...
                                            if InUse[name] then
                                                if debug then self:Debug(
                                                    "Action list (%s) was found, but would cause a loop.", name) end
                                            elseif name and pack.lists[name] then
                                                if debug then self:Debug("Action list (%s) was found.", name) end
                                                self:AddToStack(scriptID, name, caller,
                                                    not ts and action == "run_action_list")

                                                rAction, rWait, rDepth = self:GetPredictionFromAPL(dispName, packName,
                                                    name, slot, rAction, rWait, rDepth, scriptID)
                                                if debug then self:Debug(
                                                    "Returned from list (%s), current recommendation is %s (+%.2f).",
                                                        name, rAction or "NO ACTION", rWait) end
                                                ...
                                                if debug then self:Debug("Action list (%s) not found.  Skipping.",
                                                        name or "no name") end
                                            end
                                        end
                                    end
                                elseif action == "variable" then
                                    ...
                                    if class.variables[name] then
                                        if debug then self:Debug(
                                            " - variable.%s references a hardcoded variable and this entry will be ignored.",
                                                name) end
                                    elseif name ~= nil then
                                        state:RegisterVariable(name, scriptID, listName, Stack)
                                        if debug then self:Debug(
                                            " - variable.%s[%s] will check this script entry ( %s )", name,
                                                tostring(state.variable[name]), scriptID) end
                                    else
                                    ...
                                    if state.args.cycle_targets == 1 and state.settings.cycle and state.active_enemies > 1 then
                                        ...
                                        if state.cycle_enemies == 1 then
                                            if debug then Hekili:Debug(
                                                "There is only 1 valid enemy for target cycling; canceling cycle.") end
                                            state.ClearCycle()
                                        end
                                    else
                                        state.ClearCycle()
                                    ...
                                    if debug then
                                        if usable then
                                            ...
                                            if cost and cost > 0 then
                                                self:Debug(
                                                "The action (%s) is usable at (%.2f + %.2f) with cost of %d %s (have %d).",
                                                    action, state.offset, state.delay, cost or 0, costType or "unknown",
                                                    costType and state[costType] and state[costType].current or -1)
                                            else
                                                self:Debug("The action (%s) is usable at (%.2f + %.2f).", action,
                                                    state.offset, state.delay)
                                            end
                                        else
                                            self:Debug("The action (%s) is unusable at (%.2f + %.2f) because %s.", action,
                                                state.offset, state.delay, why or "IsUsable returned false")
                                        end
                                    end
                                    ...
                                    if usable then
                                        ...
                                        if debug then self:Debug(
                                            " - the action is %sready before the current recommendation (at +%.2f vs. +%.2f).",
                                                readyFirst and "" or "NOT ", state.delay, waitValue) end
                                        ...
                                        if readyFirst then
                                            ...
                                            if hasResources then
                                                local channelPass = not state.channeling or (action ~= state.channel) or
                                                self:CheckChannel(action, rWait)
                                                ...
                                                if not channelPass then
                                                    if debug then self:Debug(
                                                        " - this entry cannot break the channeled spell.") end
                                                    if action == state.channel then
                                                        stop = scripts:CheckScript(scriptID)
                                                    end
                                                elseif not aScriptPass then
                                                    if debug then self:Debug(
                                                        " - this entry would not be reached at the current time via the current action list path (%.2f).",
                                                            state.delay) end
                                                else
                                                    if not entry.criteria or entry.criteria == '' then
                                                        if debug then
                                                            self:Debug(" - this entry has no criteria to test.")
                                                            if not channelPass then self:Debug(
                                                                "   - however, criteria not met to break current channeled spell.") end
                                                        end
                                                    else
                                                        ...
                                                        if debug then
                                                            self:Debug(" - this entry's criteria %s: %s",
                                                                aScriptPass and "PASSES" or "FAILS",
                                                                scripts:GetConditionsAndValues(scriptID))
                                                        end
                                                    end
                                                end
                                                ...
                                                if not aScriptPass then
                                                    ...
                                                    if #state.recheckTimes == 0 then
                                                        if debug then self:Debug(
                                                            "There were no recheck events to check.") end
                                                    elseif cap_sec > 0 and #state.recheckTimes > 0 and state.recheckTimes[1] > cap_sec then
                                                        if debug then self:Debug(
                                                            "Recheck timing ( %.2f ) exceeds preferred Maximum Forecasting Time ( %.2f )",
                                                                state.recheckTimes[1], cap_sec) end
                                                    else
                                                        local base_delay = state.delay
                                                        local cap_n = spec.throttleForecastingCount or 0

                                                        if debug then
                                                            self:Debug("There are " ..
                                                            #state.recheckTimes .. " recheck events.")
                                                            if cap_n > 0 and cap_n < #state.recheckTimes then self:Debug(
                                                                " - the number of rechecks will be limited to preferred Maximum Forecasting Count ( %d > %d ).",
                                                                    #state.recheckTimes, cap_n) end
                                                            self:Debug("Times: " ..
                                                            table.concat(state.recheckTimes, ", "))
                                                        end
                                                        ...
                                                        for i, step in pairs(state.recheckTimes) do
                                                            ...
                                                            if cap_n > 0 and cap_n < i then
                                                                if debug then self:Debug(
                                                                    "Rechecking stopped at step #%d.  The number of rechecks exceeds the preferred Maximum Forecasting Count ( %d > %d ).",
                                                                        i - 1, #state.recheckTimes, cap_n) end
                                                                break
                                                            elseif cap_sec > 0 and new_wait > cap_sec then
                                                                if debug then self:Debug(
                                                                    "Rechecking stopped at step #%d.  The recheck time ( %.2f ) exceeds the preferred Maximum Forecasting Time ( %.2f ).",
                                                                        i, new_wait, cap_sec) end
                                                                break
                                                            ...
                                                            elseif (action ~= state.channel) and waitValue <= base_delay + step + 0.05 then
                                                                if debug then self:Debug(
                                                                    "Rechecking stopped at step #%d.  The previously chosen ability is ready before this recheck would occur ( %.2f <= %.2f + 0.05 ).",
                                                                        i, waitValue, new_wait) end
                                                                break
                                                            end
                                                            ...
                                                            if debug then
                                                                if not usable then
                                                                    self:Debug(
                                                                    "The action (%s) is no longer usable at (%.2f + %.2f) because %s.",
                                                                        action, state.offset, state.delay,
                                                                        why or "IsUsable returned false")
                                                            ...
                                                            if self:CheckStack() then
                                                                ...
                                                                channelPass = not state.channeling or
                                                                (action ~= state.channel) or
                                                                self:CheckChannel(action, rWait)

                                                                Timer:Track("Recheck Post-Channel")

                                                                if debug then
                                                                    self:Debug("Recheck #%d ( +%.2f ) %s: %s", i,
                                                                        state.delay, aScriptPass and "MET" or "NOT MET",
                                                                        scripts:GetConditionsAndValues(scriptID))
                                                                    if not channelPass then self:Debug(
                                                                        " - however, criteria not met to break current channeled spell.") end
                                                                end
                                                                ...
                                                                if debug then self:Debug(
                                                                    "Unable to recheck #%d at %.2f, as APL conditions would not pass.",
                                                                        i, state.delay) end
                                                            end
                                                            ...
                                                            if aScriptPass then
                                                                if first_rechannel == 0 and state.channel and action == state.channel then
                                                                    ...
                                                                    if debug then self:Debug(
                                                                        "This is the currently channeled spell; it would be rechanneled at this time, will check end of channel.  " ..
                                                                        state.channel_remains) end
                                                                elseif first_rechannel > 0 and (not state.channel or state.channel_remains < 0.05) then
                                                                    if debug then self:Debug(
                                                                        "Appears that the ability would be cast again at the end of the channel, stepping back to first rechannel point.  " ..
                                                                        state.channel_remains) end
                                                ...
                                                if aScriptPass then
                                                    ...
                                                    if action == "wait" then
                                                        ...
                                                        if sec <= 0 then
                                                            if debug then self:Debug(
                                                                "Invalid wait value ( %.2f ); skipping...", sec) end
                                                        else
                                                            slot.scriptType = "simc"
                                                            slot.script = scriptID
                                                            slot.hook = caller
                                                            ...
                                                            if debug then
                                                                self:Debug("Action chosen:  %s at %.2f!", rAction,
                                                                    state.delay)
                                                                self:Debug("Texture shown:  %s", slot.texture)
                                                            end
                                                        end
                                                    ...
                                                    elseif action == "pool_resource" then
                                                        if state.args.for_next == 1 then
                                                            ...
                                                            local next_action           = next_entry and
                                                            next_entry.action
                                                            local next_id               = next_action and
                                                            class.abilities[next_action] and
                                                            class.abilities[next_action].id

                                                            local extra_amt             = state.args.extra_amount or 0

                                                            local next_known            = next_action and
                                                            state:IsKnown(next_action)
                                                            local next_usable, next_why = false, "unknown"
                                                            if next_action then
                                                                next_usable, next_why = state:IsUsable(next_action)
                                                            end
                                                            local next_cost = next_action and state.action
                                                            [next_action] and state.action[next_action].cost or 0
                                                            local next_res  = next_action and
                                                            state.GetResourceType(next_action) or class.primaryResource

                                                            if not next_entry then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for non-existent next entry in the APL.  Skipping.") end
                                                            elseif not next_action or not next_id or next_id < 0 then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for invalid next entry in the APL.  Skipping.") end
                                                            elseif not next_known then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for Next Entry ( %s ), but the next entry is not known.  Skipping.",
                                                                        next_action) end
                                                            elseif not next_usable then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for Next Entry ( %s ), but the next entry is not usable because %s.  Skipping.",
                                                                        next_action, next_why or "of an unknown reason") end
                                                            elseif state.cooldown[next_action].remains > 0 then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for Next Entry ( %s ), but the next entry is on cooldown.  Skipping.",
                                                                        next_action) end
                                                            elseif state[next_res].current >= next_cost + extra_amt then
                                                                if debug then self:Debug(
                                                                    "Attempted to Pool Resources for Next Entry ( %s ), but we already have all the resources needed ( %.2f > %.2f + %.2f ).  Skipping.",
                                                                        next_action, state[next_res].current, next_cost,
                                                                        extra_amt) end
                                                            else
                                                                -- Oops.  We only want to wait if
                                                                local next_wait = state[next_res]
                                                                ["time_to_" .. (next_cost + extra_amt)]
                                                                ...
                                                                if next_wait <= 0 then
                                                                    if debug then self:Debug(
                                                                        "Attempted to Pool Resources for Next Entry ( %s ), but there is no need to wait.  Skipping.",
                                                                            next_action) end
                                                                elseif next_wait >= rWait then
                                                                    if debug then self:Debug(
                                                                        "The currently chosen action ( %s ) is ready at or before the next action ( %.2fs <= %.2fs ).  Skipping.",
                                                                            (rAction or "???"), rWait, next_wait) end
                                                                elseif state.delayMax and next_wait >= state.delayMax then
                                                                    if debug then self:Debug(
                                                                        "Attempted to Pool Resources for Next Entry ( %s ), but we would exceed our time ceiling in %.2fs.  Skipping.",
                                                                            next_action, next_wait) end
                                                                    ...
                                                                    aScriptPass = not next_entry.criteria or
                                                                    next_entry.criteria == '' or
                                                                    scripts:CheckScript(packName ..
                                                                    ':' .. listName .. ':' .. (actID + 1))
                                                                    state.this_action = "pool_resource"

                                                                    if not aScriptPass then
                                                                        if debug then self:Debug(
                                                                            "Attempted to Pool Resources for Next Entry ( %s ), but its conditions would not be met.  Skipping.",
                                                                                next_action) end
                                                                        state.offset = state.offset - next_wait
                                                                    else
                                                                        if debug then self:Debug(
                                                                            "Pooling Resources for Next Entry ( %s ), delaying by %.2f ( extra %d ).",
                                                                                next_action, next_wait, extra_amt) end
                                                                        state.offset = state.offset - next_wait
                                                            ...
                                                            if debug then self:Debug(
                                                                "pool_resource is disabled as pooling is automatically accounted for by the forecasting engine.") end
                                                        end
                                                        ...
                                                        slot.empower_to = ability.empowered and
                                                        (state.args.empower_to or ability.empowerment_default or state.max_empower) or
                                                        nil
                    ...
                    if debug and action ~= "call_action_list" and action ~= "run_action_list" and action ~= "use_items" then
                        self:Debug("Time spent on this action:  %.2fms\nTimeData:%s-%s-%d:%s:x%d:%.2f%s", Timer:Total(),
                            packName, listName, actID, action, #state.recheckTimes, Timer:Total(), Timer:Output())
                    end
                end
            else
                if debug then self:Debug("\nEntry #%d in list ( %s ) is not set or not enabled.  Skipping.", actID,
                        listName) end
            end
    ...
    if debug then self:Debug("Exiting %s with recommendation of %s at +%.2fs.", listName or "UNKNOWN",
            action or "NO ACTION", wait or state.delayMax) end
...
function Hekili:GetNextPrediction(dispName, packName, slot)
    ...
    for k, v in pairs(listCache) do
        tinsert(lcPool, v); twipe(v); listCache[k] = nil
    end
    for k, v in pairs(listValue) do
        tinsert(lvPool, v); twipe(v); listValue[k] = nil
    end
    ...
    if self.ActiveDebug then
        self:Debug("Checking if I'm casting ( %s ) and if it is a channel ( %s ).",
            state.buff.casting.up and "Yes" or "No", state.buff.casting.v3 == 1 and "Yes" or "No")
        if state.buff.casting.up then
            if state.buff.casting.v3 == 1 then self:Debug(" - Is criteria met to break channel?  %s.",
                    state.channel_breakable and "Yes" or "No") end
    ...
    if not state.channel_breakable and state.buff.casting.up and state.spec.can_dual_cast then
        self:Debug("Whitelist of castable-while-casting spells applied [ %d, %.2f ]", state.buff.casting.v1,
    ...
    if pack.lists.precombat then
        ...
        if debug then
            self:Debug(1, "\nProcessing precombat action list [ %s - %s ].", packName, listName); self:Debug(2, "")
        end
    ...
    if pack.lists.default and wait > 0 then
        ...
        if debug then
            self:Debug(1, "\nProcessing default action list [ %s - %s ].", packName, listName); self:Debug(2, "")
        end
        ...
        if UI.Active and UI.alpha > 0 and rule(profile) then
            ...
            for i = 1, numRecs do
                ...
                if debug then Hekili:Debug(0, "\nRECOMMENDATION #%d ( Offset: %.2f, GCD: %.2f, %s: %.2f ).\n", i,
                        state.offset, state.cooldown.global_cooldown.remains,
                        (state.buff.casting.v3 == 1 and "Channeling" or "Casting"), state.buff.casting.remains) end
                ...
                if isMain and #events > 0 then
                    ...
                    while (event) do
                        ...
                        if t < 0 then
                            ...
                            if debug then Hekili:Debug(1,
                                    "Finishing queued event #%d ( %s of %s ) due at %.2f because the event should've already occurred.\n",
                            ...
                            if channeling then
                                if debug then Hekili:Debug(
                                    "We are channeling, checking if we should break the channel...") end
                            ...
                            if (casting or (channeling and not shouldBreak)) and state.spec.can_dual_cast then
                                ...
                                for spell in pairs(state.spec.dual_cast) do
                                    if debug then Hekili:Debug("CWC: %s | %s | %s | %s | %.2f | %s | %.2f | %.2f", spell,
                                            tostring(state:IsKnown(spell)), tostring(state:IsUsable(spell)),
                                            tostring(class.abilities[spell].dual_cast), state:TimeToReady(spell),
                                            tostring(state:TimeToReady(spell) <= t), state.offset, state.delay) end
                                ...
                                if eventAbility and not eventAbility.tick then
                                    ...
                                    for i = 1, #events do
                                        ...
                                        if e.type ~= "CHANNEL_TICK" then
                                            ...
                                            if debug then Hekili:Debug(
                                                "As %s's channel has no tick function, we will check between now and %s's %s event in %.2f seconds.",
                                                    event.action, overrideAction, overrideType, overrideTime) end
                                            break
                                        end
                                    end
                                end
                            ...
                            if (casting or channeling) and not shouldBreak and not shouldCheck then
                                if debug then Hekili:Debug(1,
                                        "Finishing queued event #%d ( %s of %s ) due at %.2f as player is casting and castable spells are not ready.\nCasting: %s, Channeling: %s, Break: %s, Check: %s",
                                        n, event.type, event.action, t, casting and "Yes" or "No",
                                        channeling and "Yes" or "No", shouldBreak and "Yes" or "No",
                                        shouldCheck and "Yes" or "No") end
                                if t >= 0 then
                                    state.advance(t)
                                ...
                                if debug then Hekili:Debug(1,
                                        ...
                                        overrideIndex or n, overrideAction or event.action, overrideType or event.type,
                                        overrideTime or t) end

                                if casting or channeling then
                                    state:ApplyCastingAuraFromQueue()
                                    if debug then Hekili:Debug(2, "Player is casting for %.2f seconds.  %s.",
                                            state.buff.casting.remains,
                                            shouldBreak and "We can break the channel" or
                                            "Only spells castable while casting will be used") end
                                else
                                ...
                                repeat
                                    ...
                                    if action == "wait" then
                                        if debug then Hekili:Debug(
                                            "EXECUTING WAIT ( %.2f ) EVENT AT ( +%.2f ) AND RECHECKING RECOMMENDATIONS...",
                                                slot.waitSec, wait) end
                                ...
                                if not action then
                                    if debug then Hekili:Debug("Time spent on event #%d PREADVANCE: %.2fms...", n,
                                            debugprofilestop() - eStart) end
                                    if debug then Hekili:Debug(1,
                                            "No recommendation found before event #%d (%s %s) at %.2f; triggering event and continuing ( %.2f ).\n",
                                            n, event.action or "NO ACTION", event.type or "NO TYPE", t,
                                            state.offset + state.delay) end

                                    state.advance(overrideTime or t)
                                    if debug then Hekili:Debug("Time spent on event #%d POSTADVANCE: %.2fms...", n,
                                            debugprofilestop() - eStart) end
                        ...
                        if n > 10 then
                            if debug then Hekili:Debug(
                                "WARNING:  Attempted to process 10+ events; breaking to avoid CPU wastage.") end
                            break
                        end
                ...
                if not action then
                    ...
                    elseif state.spec.assassination then
                        ...
                        state:SetConstraint(0,
                            max(state.delayMax, 0.01 + state.energy.max / (state.energy.regen_combined or 0.001)))
                    end

                    if hadProj and debug then Hekili:Debug(
                ...
                if debug then
                    Hekili:Debug("Recommendation #%d is %s at %.2fs (%.2fs).", i, action or "NO ACTION",
                        wait or state.delayMax, state.offset + state.delay)
                end
                ...
                if action then
                    ...
                    local ability = class.abilities[action]
                    local cast_target = i == 1 and state.cast_target ~= "nobody" and state.cast_target or
                    state.target.unit
                    ...
                    if i < display.numIcons then
                        ...
                        if cast > 0 then
                            if not ability.channeled then
                                if debug then Hekili:Debug("Queueing %s cast finish at %.2f [+%.2f] on %s.", action,
                                        state.query_time + cast, state.offset + cast, cast_target) end
                                ...
                                if debug then Hekili:Debug("Queueing %s channel finish at %.2f [%.2f+%.2f].", action,
                                        state.query_time + cast, state.offset, cast, cast_target) end
                                ...
                                if ability.tick_time then
                                    ...
                                    for i = 1, ticks do
                                        state:QueueEvent(action, state.query_time,
                                            state.query_time + (i * ability.tick_time), "CHANNEL_TICK", cast_target)
                                    end
                                ...
                                if Hekili.Scripts.Channels and Hekili.Scripts.Channels[packName] and Hekili.Scripts.Channels[packName][action] then
                                    state:QueueEvent(action, state.query_time, state.cooldown.global_cooldown.expires,
                                        "GCD_FINISH", cast_target)
                                end
                    ...
                    if debug then
                        ...
                        for k in orderedPairs(class.resources) do
                            ...
                            if res then
                                ...
                                if forecast then
                                    final = string.format("%.2f @ [%d - %s] %.2f", forecast.v, res.fcount,
                                        forecast.e or "none", forecast.t - state.now - state.offset)
                                end

                                local okc, cur = pcall(function() return res.current end); if not okc then cur = rawget(
                                    res, "actual") or 0 end
                                local okm, maxv = pcall(function() return res.max end); if not okm then maxv = rawget(
                                    res, "max") or 0 end

                                resInfo = (resInfo and (resInfo .. ", ") or "") ..
                                string.format("%s[ %.2f / %.2f || %s ]", k, cur or 0, maxv or 0, final)
                            end
                        ...
                        if i < 5 and not hasSnapped and profile.autoSnapshot and InCombatLockdown() and state.level >= 70 and (dispName == "Primary" or dispName == "AOE") and state.this_action ~= "wait" then
                            Hekili:Print("Unable to make recommendation for " ..
                            dispName .. " #" .. i .. "; triggering auto-snapshot...")
                            hasSnapped = dispName
            ...
            if WeakAuras and WeakAuras.ScanEvents then
                ...
                for i = 1, numRecs do
                    if UI.EventPayload[i] then
                        wipe(UI.EventPayload[i])
                    else
                        UI.EventPayload[i] = {}
                    end
                ...
                WeakAuras.ScanEvents("HEKILI_RECOMMENDATION_UPDATE", dispName, Queue[1].actionID, Queue[1].indicator,
                    Queue[1].empower_to, UI.EventPayload)
            end
...
function Hekili:DumpFrameInfo()
    ...
    for i, v in ipairs(usedCPU) do
        if v.peak and type(v.peak) == "number" then
            print(format("%-40s %6.2fms (%6d calls, %6.2fms average, %6.2fms peak)", v.name, v.usage, v.calls, v.average,
                v.peak))
        else
