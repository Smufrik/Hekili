...
spec:RegisterSetting( "hold_sc", false, {
    name = strformat("Hold %s charge", Hekili:GetSpellLinkWithTexture( spec.abilities.shadow_crash.id )),
    desc = strformat("If checked, the addon will always keep one chearge of %s available for use.", Hekili:GetSpellLinkWithTexture( spec.abilities.shadow_crash.id )),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "hold_sc_toggle", "none", {
    name = strformat("%s charge toggle", Hekili:GetSpellLinkWithTexture( spec.abilities.shadow_crash.id )),
    desc = strformat("If 'Hold %s charge' is checked, the addon will only keep a charge if this toggle is active at the same time.", Hekili:GetSpellLinkWithTexture( spec.abilities.shadow_crash.id )),
    type = "select",
    width = "full",
    values = function ()
        local toggles = {
